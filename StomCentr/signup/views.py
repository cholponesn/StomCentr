...
class OrderViewSet(ViewSet):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                if work_day.status == 'free':
                    ...
                    work_day.save()
                    return Response(serializer.data, status=201)
                return Response('Это время уже занято!', 400)
    ...
    def update(self, request, order_id):
        ...
        if serializer.is_valid():
            ...
            try:
                dd = DoctorDay.objects.get(day=day_id, doctor=doctor_id,
                                           status='free')  # SELECT * FROM DOCTORDAY WHERE DAY=DAY_ID AND DOCTOR=DOCTOR_ID AND STATUS=FREE
                dd.status = 'reserved'
                dd.save()
            except DoctorDay.DoesNotExist:
...
class DoctorViewSet(ViewSet):
    ...
    def updata(self,request,d_username):
        pass
